...
> **优先级**：P0  
> **实现技术**：Flutter 原生  
> **原型**：![App端NPS问卷入口](../../assets/prototype/App端NPS问卷入口.png)
...
| --- | --------- | --- | ------------------------------- | ---------- |
| 车控页 | 点击 NPS 浮标 | 问卷页 | survey URL（已含 ?oneId=xxx 参数）、campaignCode、position | WebView 打开 |
| 车控页 | 点击关闭      | 车控页 | -                               | 浮标隐藏       |
...
- **BR-05.04-01** 客户端在获取车辆信息完成后调用问卷查询接口，查询当前用户的有效问卷数据；返回的 `url` 已由后端拼接 `?oneId=xxx`（或 `&oneId=xxx`）参数。可返回多条不同位置的投放记录。（→ AC-05.04-01）
- **BR-05.04-02** 问卷数据返回规则：投放计划启用、当前用户存在已下发且未完成的问卷、当前时间在用户投放时间段内。（→ AC-05.04-02）
...
| 车型 | model | string | 否 | - | |
| 问卷 URL | url | string | 是 | URL | 已由后端拼接 ?oneId=xxx 参数 |
  ...
  "data": [
    {
      ...
      "model": "AY5-G",
      "url": "https://csi.example.com/survey/1007?oneId=one192929293939",
      "coolOff": "24",
