...
- **BR-05.01-05** 投放计划支持启用/停用。停用后：① 不再参与新的定时下发；② App 问卷查询接口（`GET /api/v1/campaigns/user/surveys`）不再返回该计划的投放数据（含停用前已生成的投放记录），App 端不再展示该投放入口。已生成的投放记录数据仍保留（管理后台投放数据管理不受影响），仅 App 侧不再返回与展示；计划重新启用后，符合返回条件的投放记录恢复返回与展示。用户已打开问卷页的提交不受影响，完成上报接口照常接收（见 BR-05.05-08）。（→ AC-05.01-05）
...
|----------|----------|----------|----------|--------|
| 新建/停用 | ENABLED | 启用计划 | 运营人员 | 次日定时任务可参与下发；App 查询接口恢复返回该计划投放数据 |
| ENABLED | DISABLED | 停用计划 | 运营人员 | 不再执行新下发；App 查询接口不再返回该计划投放数据，已下发记录保留不删除 |
...
- **AC-05.01-05**: **Given** 计划处于启用状态且已生成用户投放记录，**When** 运营人员点击停用，**Then** 计划状态变为停用，次日定时任务不再下发该计划，且 App 问卷查询接口不再返回该计划的投放数据、App 端不再展示该投放入口。（← BR-05.01-05）
