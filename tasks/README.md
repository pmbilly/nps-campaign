...
| 1.5 启用/停用 | Switch 切换，停用后不下发，App 查询接口不再返回、App 端不再展示 | AC-05.01-05 |
...
| 子任务 | 描述 | 关键 AC |
|--------|------|---------|
| 3.1 问卷查询 | 获取车辆信息后调用问卷查询接口；停用计划的投放记录不返回 | AC-05.04-01, AC-05.04-10 |
...
| 4.5 后台状态同步 | 异步调用完成接口；失败则本地存 pending 记录，每次 App 启动重试，超 1 小时最后一次重试后失效；计划停用不影响接收 | AC-05.05-07, AC-05.05-08, AC-05.05-11 |
